(() => {
  ...
  const riskData = () => {
    const s = state();
    const lead = {...DEMO_PATIENTS[0], note:s.red?'语音随访命中红色风险；患者已被提示就近急诊':DEMO_PATIENTS[0].note, action:'查看风险卡'};
    return [lead, {...DEMO_PATIENTS[1], action:'查看风险卡'}];
  };
  const header = (title, sub='汤立晨主任团队 · 化疗随访', back='home', right='patient') => `<header class="wbv2-head"><button class="wbv2-back" data-go="${back}" aria-label="返回工作台">‹</button><div><h1>${title}</h1><small>${sub}</small></div><button class="wbv2-patient" data-go="${right}" aria-label="返回患者端">${right==='logs'?'日志':'↗'}</button></header>`;
  ...
  function riskDetail() {
    ...
    const current = p.name==='陈女士' && s.voiceDone;
    ...
    const feverRecord = current ? summaryValue('体温与感染征象') : '';
    const highest = feverRecord.match(/(?:体温)?\s*(3[789]|40)(?:[.点][0-9一二三四五六七八九])?/)?.[0] || '38.8℃';
    ...
    const riskText = p.name==='陈女士' ? `D6 语音随访中记录最高体温 ${highest}；${symptom}` : p.note;
  ...
  function bind() {
    ...
    ROOT.querySelectorAll('[data-contact]').forEach(el => el.onclick = () => { const s=state(); s.contacted=true; localStorage.setItem(KEY,JSON.stringify(s)); notice('已记录“团队已联系患者”（演示）。'); render(); });
